...
-- Test suite for gather_input_files_to_watch
...
describe("get_do_watch", function()
	...
	it("returns an error if no watcher is available", function()
		local options = { watch = "auto" }
		-- Temporarily mock shellutil.has_command to return false
		local original_has_command = shellutil.has_command
		shellutil.has_command = function() return false end

		expect.fail(function()
			watcher.get_do_watch(options)
		end)

		-- Restore original function
		shellutil.has_command = original_has_command
	end)
end)
